...
2. High: the new stop-reply dedupe window can ignore a legitimate second `/stop` and leave fresh work running. `handleStop()` now returns immediately on any chat with a recent stop marker (`super_turtle/claude-telegram-bot/src/handlers/stop.ts:151-155`), and every successful foreground stop records that marker for two seconds (`super_turtle/claude-telegram-bot/src/handlers/stop.ts:229-234`). There is no run identifier or active-state check attached to that fast path, so a user who stops one run, immediately starts another, and then sends `/stop` again inside the 2s window will skip `stopForegroundWork()` entirely. The dedicated test added for this behavior (`super_turtle/claude-telegram-bot/src/handlers/stop.test.ts:498-560`) codifies the unconditional suppression, but it never exercises the “new run started before the dedupe window expired” case.

3. Medium: ask-user callback runs do not preserve retained progress on explicit stop the way normal text runs do. In `handleCallback()`, every driver exception tears the progress state down first (`super_turtle/claude-telegram-bot/src/handlers/callback.ts:557-569`) and only then checks whether the error was an interrupt/stop (`super_turtle/claude-telegram-bot/src/handlers/callback.ts:571-577`). `handleText()` already has the opposite shape: it detects user-initiated cancellation and converts the active progress card into a retained `Stopped` snapshot before teardown (`super_turtle/claude-telegram-bot/src/handlers/text.ts:215-241`). That mismatch leaves callback-driven ask-user continuations exposed to the same late-progress race as the main text path and can also drop the retained stopped card entirely if the callback catches cancellation before `handleStop()` finishes its own retention path. Current callback coverage only exercises snapshot navigation (`super_turtle/claude-telegram-bot/src/handlers/callback.test.ts:65-232`); there is no regression test for a callback run being stopped mid-stream.

4. Medium: the snapshot filter now drops the required `Still working` history entry, so the retained viewer loses heartbeat-state navigation. `startHeartbeat()` explicitly asks to store a snapshot when entering `Still working` at `super_turtle/claude-telegram-bot/src/handlers/streaming.ts:1626-1632`, but `recordProgressSnapshot()` now returns early for every non-terminal state except `Writing answer` at `super_turtle/claude-telegram-bot/src/handlers/streaming.ts:1169-1173`. That contradicts the active UX spec, which still requires storing the heartbeat transition into `Still working` in the snapshot history (`super_turtle/docs/TELEGRAM_PROGRESS_UX_SPEC.md:136-143`). A simple probe (`thinking` -> 20s idle heartbeat -> `done`) produced only a terminal `Done` snapshot, so the viewer can no longer page back to the idle-state transition, and the updated tests no longer cover that regression.

## Verification

- `bun test super_turtle/claude-telegram-bot/src/handlers/stop.test.ts super_turtle/claude-telegram-bot/src/handlers/callback.test.ts super_turtle/claude-telegram-bot/src/handlers/commands.subturtle.test.ts`
