...
- [ ] Fix #11: start-tunnel.sh lines 177-187 — remove the two dead subshell blocks that try to `wait` on parent's children (can't work from a subshell). Remove DEV_WAIT_PID and TUNNEL_WAIT_PID variables. The `kill -0` polling loop on line 190 already handles monitoring. <- current
