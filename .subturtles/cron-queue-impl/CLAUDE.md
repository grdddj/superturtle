...
- [ ] Update `src/index.ts` cron timer idle path: after processing due jobs, call `drainDeferredQueue()` for queued cron items so they run even without a new user message arriving. <- current
