...
- [ ] Create `linkedin-demo/src/convex/likes.ts`: export `toggleLike` mutation (args: userId, postId). Check if a like exists for this user+post pair using `ctx.db.query("likes").filter(...)`. If exists, delete it and decrement `likesCount` on the post. If not, insert and increment `likesCount`. Also export `getLikeStatus` query (args: userId, postId) that returns boolean. <- current
