# Current task
Search `super_turtle/claude-telegram-bot/src/` for any remaining imports or references to `ORCHESTRATOR_PROMPT` or `orchestrator` mode and clean up stale references. Commit after each change.
...
- [ ] Search entire `super_turtle/claude-telegram-bot/src/` for any remaining imports or references to `ORCHESTRATOR_PROMPT` or `orchestrator` mode — clean up any stale references (config.ts export is already removed) <- current
- [ ] Remove orchestrator code from `super_turtle/subturtle/ctl`: delete `build_orchestrator_prompt()` function, remove `--cron-mode` flag and its argument parsing, remove the orchestrator branch in `register_spawn_cron_job()`. Keep silent mode as the only supervision mode.
