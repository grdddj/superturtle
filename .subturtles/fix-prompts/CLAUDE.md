# Current task
Fix Codex-meta#3: update `super_turtle/meta/META_SHARED.md` so any guidance about directly editing `.superturtle/cron-jobs.json` explicitly says manual JSON edits are only for recovery/debug, while normal scheduling should use `ctl spawn` or the CronCreate/CronDelete tools.
