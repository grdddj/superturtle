# Current task
Grep for remaining `chat_id` references in cron.ts, conductor-maintenance.ts, dashboard-types.ts, dashboard/data.ts.
...
- Grep for remaining `chat_id` references in cron.ts, conductor-maintenance.ts, dashboard-types.ts, dashboard/data.ts <- current
